...
export const uploadFile1 = (file, onUploadProgress) => {
  ...
  return axios.post('https://ggafwzcegl.execute-api.ap-northeast-2.amazonaws.com/production/upload_excel/', formData, {
    headers: {
...
export const uploadFile3 = (file, onUploadProgress) => {
  ...
  return axios.post('https://ggafwzcegl.execute-api.ap-northeast-2.amazonaws.com/production/upload_margin/', formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
      'Authorization': `Bearer ${token}`  // Authorization 헤더에 토큰 추가
    },
