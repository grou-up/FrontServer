...
export const uploadFile1 = (file, onUploadProgress) => {
  ...
  return axios.post('https://server.grouup.co.kr/django/upload_excel/django/upload_excel/', formData, {
    headers: {
...
export const uploadFile3 = (file, onUploadProgress) => {
  ...
  return axios.post('https://server.grouup.co.kr/django/upload_excel/upload_margin/', formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
      'Authorization': `Bearer ${token}`  // Authorization 헤더에 토큰 추가
    },
